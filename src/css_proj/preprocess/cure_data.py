...
INTERMEDIATE_PATH = "./data/data_processed/cure_data_intermediate.pkl"


def get_empty_df_cure(path: str = CURE_DATA_PATH, save: bool = True):
    """
    ...
    if save:
        save_intermediate_df_cure(df)
    return df


def save_intermediate_df_cure(df, path: str = INTERMEDIATE_PATH):
    """
...
def load_intermediate_df_cure(path: str = INTERMEDIATE_PATH):
    """
...
def get_df_cure(api, path: str = CURE_DATA_PATH, start_idx: int = 0, end_idx: int = -1):
    """
    ...
    path (str): The path to the COVID-19 lies dataset (csv).
    """
    df = get_empty_df_cure(path, save=True)
    if end_idx == -1:
        end_idx = len(df)
...
def clean_up_intermediate(
    ...
    save_path="./data/data_processed/cure_data_cleaned.pkl",
):
    """
    ...
    path (str): The path to the intermediate dataset.
    """
    df = load_intermediate_df_cure(path)
    df = df[df["state"] == 1]
